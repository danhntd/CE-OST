...
class Visualizer:
    ...
    def overlay_instances(
        ...
        *,
        ...
        bbx_colors = [random_color(rgb=True, maximum=1) for _ in range(num_instances)]
        for i in range(num_instances):
            color = assigned_colors[i]
            if boxes is not None:
                # self.draw_box(boxes[i], edge_color=color)
                self.draw_box(boxes[i], edge_color=bbx_colors[i])

            if masks is not None:
                for segment in masks[i].polygons:
                    self.draw_polygon(segment.reshape(-1, 2), color, bbx_colors[i], alpha=alpha)

            if labels is not None:
                # first get a box
                if boxes is not None and boxes[i][0] !=0:
                    x0, y0, x1, y1 = boxes[i]
